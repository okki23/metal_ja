   ...
   <script type="text/javascript">
	 ...
	 function Show_Detail(id){ 
		...
		$.ajax({
			 url:"<?php echo base_url(); ?>struktur/get_data_edit/"+id,
			 type:"GET",
			 dataType:"JSON", 
			 success:function(result){  
                 ...
                 $("#no_struktur_dtl").html(result.no_struktur); 
				 $("#lantai_dtl").html(result.lantai); 
				 $("#harga_dtl").html(result.harga);
				 $("#luas_dtl").html(result.luas);
				 $("#foto_struktur_dtl").attr("src","upload/"+result.foto);
	 ...
	 function Hapus_Data(id){
        ...
        $.ajax({
            url : "<?php echo base_url('struktur/hapus_data')?>/"+id,
            type: "GET",
            dataType: "JSON",
	...
	function Simpan_Data(){
		//setting semua data dalam form dijadikan 1 variabel 
         ...
         if(caption_a == ''){
            ...
            $.ajax({
             url:"<?php echo base_url(); ?>struktur/simpan_data",
             type:"POST",
             data:formData,
             contentType:false,  
             processData:false,   
	 ...
	 $('.datepicker').bootstrapMaterialDatePicker({
       ...
       $(document).ready(function() {
		...
		$('#example').DataTable( {
			"ajax": "<?php echo base_url(); ?>struktur/fetch_struktur" 
		});
